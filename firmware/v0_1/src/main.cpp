...
int  TESP_CP_TIMEOUT = 0; // test cp timeout
...
void setup() {
  ...
  const char* menuhtml = "<form action='/custom' method='get'><button>Heater Control</button></form><br/>\n";
...
*/
  ...
  wm.setCountry("NZ"); // crashing on esp32 2.0

  // set Hostname

  wm.setHostname(("AE_"+wm.getDefaultAPName()).c_str());
  ...
  if(!wm.autoConnect("AE-AP","12345678")) {
